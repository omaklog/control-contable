...
- **Actualización 2026-07-17 (alineación con `001-business-domain-model` y `docs/ux/design-system.md`, sin cambio de alcance funcional)**: FR-003 se actualizó a la nomenclatura de módulos vigente (Clientes/Servicios/Cobranza/Gestión Fiscal/Gestión Documental Fiscal); FR-009 se aclaró para distinguir la auditoría de acceso (esta feature) de la futura auditoría de negocio de Cliente 360; se agregó el Acceptance Scenario 9 de la Historia 3 (consultar el registro de auditoría, sin FR/SC nuevos — ya cubierto por FR-009/SC-006); se documentaron en Assumptions las capacidades ya pre-provisionadas (`manage_billing`/`view_billing`/`manage_documents`/`view_documents`) y la ambigüedad sin resolver de `manage_catalogs`. También se corrigió `contracts/role-permissions.md` (capacidades reales en vez de categorías abstractas) y se agregaron dos gaps de UX no detectados antes a `docs/ux/design-system.md` §10 (row-actions hover y Chip de estado en `UsuariosClient.tsx`). Re-validados todos los ítems del checklist: siguen pasando 16/16 sin regresiones.
- **Actualización 2026-07-17 (pasada de `/speckit-clarify`)**: confirmado que no quedaban marcadores `[NEEDS CLARIFICATION]`. Se detectó el único Edge Case que nunca tuvo respuesta desde la redacción original (múltiples intentos fallidos de inicio de sesión / fuerza bruta) — se resolvió apoyándose en el rate-limiting ya incorporado en Supabase Auth (GoTrue), sin FR nuevo ni lógica de bloqueo propia; se actualizó el Edge Case y se agregó una nota en Assumptions. 16/16 items siguen pasando.
- **Actualización 2026-07-17 (pasada de `/speckit-plan`)**: `plan.md`/`research.md`/`quickstart.md` se actualizaron para el único ajuste con superficie de código real detectado en las sesiones anteriores — `UsuariosClient.tsx` (Chip de estado + row-actions hover, research.md #14) — sin cambios de alcance, datos ni FR/SC nuevos (por eso no aplica a este checklist de calidad del spec). 16/16 items siguen pasando.
