...
3. **Given** no existe ningún cliente registrado todavía, **When** el personal abre el listado, **Then** ve un estado vacío claro que lo invita a agregar el primer cliente.
...
6. **Given** el personal con capacidad `view_clients` o `manage_clients` está en la página de inicio del Panel Administrativo, **When** la consulta, **Then** ve un botón "Clientes" que lo lleva al listado.
...
- ¿Qué pasa si no existe ningún cliente registrado todavía? El listado muestra un estado vacío que invita a agregar el primero (ver US1, AS3).
...
- **FR-016**: El sistema DEBE agregar, en la página de inicio del Panel Administrativo, un botón "Clientes" que enlace al listado paginado, visible solo si el usuario tiene la capacidad `view_clients` o `manage_clients` — mismo patrón ya usado para los botones "Gestión de usuarios" y "Auditoría".
- **FR-017**: La entrada "Clientes" del menú de `apps/portal` (marcada "Próximamente") DEBE actualizar su texto/tooltip para aclarar que la edición y baja de clientes ya están disponibles desde el Panel Administrativo, aunque el alta desde el portal siga pendiente de una feature futura.
