...
- **Actualización 2026-07-18 (bugfix detectado al refinar `007-alta-cliente-portal`)**: la política RLS de `business_audit_log` restringía el `SELECT` exclusivamente a Administrador, contradiciendo `docs/ux/design-system.md` §9.2 (Cliente 360 permite a Auxiliar consultar la pestaña Auditoría en solo lectura). Corregido con la migración `20260718100000_business_audit_log_select_staff.sql` + prueba de integración nueva (`packages/utils/src/businessAuditLog.integration.test.ts`, T049-T052 de `tasks.md`). `contracts/db-functions-rls.md` y `data-model.md` actualizados en consistencia. Sin cambio de alcance funcional. 16/16 items siguen pasando.
