...
| `contacto_estado`  | `activo`, `obsoleto`                          | `contactos.estado`       |

**Actualización 2026-07-18**: `contacto_estado` agregado por `008-contactos-y-detalle-cliente` (migración `20260717120000_contactos_estado_principal.sql`) — no existía en la versión original de esta tabla.
...
**Actualización 2026-07-18**: agregadas las columnas `estado`/`es_principal` (`008-contactos-y-detalle-cliente`, migración `20260717120000_contactos_estado_principal.sql`) — ausentes en la versión original de esta tabla, que databa de antes de esa feature.
...
| `estado`                                               | `contacto_estado` | not null, default `activo` — soft-delete, nunca `DELETE` físico (FR-023, `008` FR-006)     |
| `es_principal`                                         | `boolean`         | not null, default `false`; a lo sumo uno en `true` por `cliente_id` (FR-026, `008` FR-007) |
| `created_at`, `updated_at`, `created_by`, `updated_by` | —                 | trazabilidad estándar                                                                      |

Índice único parcial `contactos_principal_unico` sobre `(cliente_id) where es_principal` — garantiza a lo sumo un Contacto principal por Cliente incluso ante escrituras concurrentes (autoridad real ante una carrera entre dos solicitudes).
...
**Nota (2026-07-18, `spec.md` Assumptions)**: `concepto` es hoy texto libre. Se anticipa un FK opcional futuro a un catálogo de Servicios (cuando `001-business-domain-model` lo especifique) — preservando el snapshot inmutable que ya usa `recibos.concepto` (FR-025) para no alterar cargos/recibos históricos. No aplicado en esta sesión.
...
| `tamano_bytes`                                         | `bigint`           | not null, `check (tamano_bytes > 0 and tamano_bytes <= 20971520)` — 20 MB fijo (FR-016, actualizado 2026-07-18: no es configurable desde UI, ver `packages/utils/src/expedientes.ts#TAMANO_MAXIMO_DOCUMENTO_BYTES`) |
...
**Nota (2026-07-18, `spec.md` Assumptions)**: `documentos`/`categorias_documento` solo se relacionan con `cliente_id` hoy. Se anticipa una relación opcional futura a un Periodo Fiscal (cuando `001-business-domain-model` especifique Gestión Fiscal), sin alterar el expediente ya construido. No aplicado en esta sesión.
