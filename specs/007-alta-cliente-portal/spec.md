...
- Q: ¿La columna "Estado" de la tabla del portal sigue la regla de Chip semántico de `design-system.md` (§4)? → A: No todavía — muestra texto plano ("Activo"/"Inactivo"), mismo gap ya encontrado en `ClientesClient.tsx` (`006`) y `UsuariosClient.tsx` (`003`, ya resuelto ahí). Se registra en `docs/ux/design-system.md` §10 como pendiente, sin aplicar el cambio de código en esta sesión (sin cambio de alcance funcional).
- Q: `docs/ux/design-system.md` §2.3 anticipa un buscador global en el AppBar ("Buscar clientes, RFC o documentos…"), ya diferido por `004-portal-main-layout` — ¿el filtro de nombre/RFC de esta feature entra en conflicto con ese buscador futuro? → A: No, son cosas distintas — se deja una nota en Assumptions para que quien especifique el buscador global sepa que este filtro de página ya existe y decida si lo reemplaza o lo complementa.
...
- **(2026-07-18)** `ClientesPortalClient.tsx` no sigue la regla de Chip semántico de `docs/ux/design-system.md` §4 para su columna "Estado" (texto plano en vez de un Chip azul/gris) — mismo gap ya registrado para `ClientesClient.tsx` (`006`) en `docs/ux/design-system.md` §10 (punto 8). Sin cambio de alcance funcional aquí; el seguimiento vive en ese documento.
- **(2026-07-18)** El filtro de nombre/RFC de esta feature (FR-002) es una solución acotada a esta tabla, distinta del buscador global de `apps/admin`/`apps/portal` que `docs/ux/design-system.md` §2.3 anticipa en el `AppBar` ("Buscar clientes, RFC o documentos…") y que `004-portal-main-layout` ya dejó explícitamente diferido. Quien especifique ese buscador global deberá decidir si reemplaza este filtro de página o lo complementa — no es una decisión de esta especificación.
