...
1. **Row-actions: migrar a iconos siempre visibles**: `ClientesClient.tsx` y `ClienteDetalleClient.tsx` muestran botones de acción **de texto** de forma permanente. **Actualización 2026-07-18**: la sección 5 ya NO pide ocultarlos hasta hover (esa versión de la regla se descartó tras probarse en `UsuariosClient.tsx` — resultó confusa). La migración pendiente es de formato, no de visibilidad: reemplazar los botones de texto por `IconButton` + `Tooltip` (siempre visibles) + `TableRow hover` para el fondo de fila — mismo patrón ya resuelto en `UsuariosClient.tsx` (punto 5 más abajo).
2. **Tema MUI compartido basado en este design system**: los componentes ya construidos usan los colores por defecto de Material UI, no la paleta navy/azul/slate de este documento. Crear un `ThemeProvider` compartido en `packages/ui` (definición del tema — fuera del alcance de "no generar código" de este documento) que centralice colores, tipografía, espaciado y radios de las secciones 1–8, y que ambas apps consuman sin duplicar valores.
...
7. **Badge/Chip de estado en `ClientesClient.tsx`** (`006-crud-clientes-admin`): **agregado 2026-07-18**, mismo gap que el punto 6 pero no detectado hasta ahora — la columna "Estado" muestra el texto plano "Activo"/"Inactivo" en vez de un Chip semántico (§4/§1.1: azul=activo, gris=inactivo). Aplicar la misma implementación ya resuelta en `UsuariosClient.tsx` cuando se refactorice esta tabla.
8. **Badge/Chip de estado en `ClientesPortalClient.tsx`** (`007-alta-cliente-portal`): **agregado 2026-07-18** — mismo gap que el punto 7, en la tabla de clientes de `apps/portal` (columna "Estado" en texto plano). Aplicar la misma implementación ya resuelta en `UsuariosClient.tsx` cuando se refactorice esta tabla.
9. **Badge/Chip de estado en `ClienteDetalleClient.tsx`** (`008-contactos-y-detalle-cliente`): **agregado 2026-07-18** — mismo gap que los puntos 6-8, en dos lugares de esta pantalla: el "Estado" del propio Cliente (sección "Datos generales") y el "Estado" de cada Contacto en su tabla (ambos texto plano). Aplicar la misma implementación ya resuelta en `UsuariosClient.tsx` — coordinar con el punto 4 (migración a Cliente 360), ya que probablemente se resuelvan en el mismo refactor.
